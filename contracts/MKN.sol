...
contract MKN is IERC20, Ownable {
    ...
    string private _name = "MKN";
    string private _symbol = "MKN";
    ...
    uint256 public burnNumber = 1e18;
    uint256 public boardPct = 80;
    uint256 public lpPct = 20;
    uint256 public techPct = 100;
    ...
    uint256 public total = 0.5 * 1e8 * 1e18;

    event SwapAndLiquify(uint256 tokensSwapped, uint256 usdtReceived, uint256 tokensIntoLiqudity);

    constructor (address _usdt, address _router, address _lpAddress, address _techAddress) {
        usdt = IERC20(_usdt);
        ...
        _mint(owner(), total.mul(99).div(100).add(1e18));
    ...
    function sendFees(address sender, uint256 _amount) internal returns(uint256){
        ...
        uint256 toTech = techPct.mul(_amount).div(1000);
        uint256 toFee = toBoard.add(toLp);
        
        if(poolList[sender]){
            toFee = toFee.add(toTech);
            _balances[techAddress] = _balances[techAddress].add(toTech);
            emit Transfer(sender, techAddress, toTech);
        }
        ...
        // try board.allocate(0, toBoard) {} catch {}
        board.allocate(0, toBoard);
